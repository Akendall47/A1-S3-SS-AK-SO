        ...
        if ((line[i] == ';' && parenthesis_count == 0) || line[i] == '\0')
        {
            ...
            if (strlen(token) > 0)
            {
                ...
                else if (command_has_pipes(token))
                {
                    ...
                    if (n > 0)
                        launch_pipeline(stages, n, pipe_bg);
                }
                else
                {
                    parse_command(token, args, &argsc, &is_bg);
...
void read_command_line(char line[], size_t length){
    ...
    History *current_hist = NULL; // Track current pos

    size_t pos = 0;
    size_t cursor = 0;

    memset(line, 0, length); // Ensure empty line
    ...
    while (1){
        ...
        if (c == '\n'){
            ...
            if (cursor > 0){
                ...
                clear_and_redraw(shell_prompt, line, pos);
            ...
            if (read(STDIN_FILENO, &seq, 2) == 2){
                if (seq[0] == '['){
                    if (seq[1] == 'D'){ // Left arrow
                        ...
                        if (current_hist && current_hist->next) {
                            current_hist = current_hist->next;
                            strncpy(line, current_hist->line, length - 1);
                            line[length - 1] = '\0';
                            pos = strlen(line);
                        }else if (current_hist) {
                            current_hist = NULL;
                            line[0] = '\0';
                            pos = 0;
                        }

                        cursor = pos; // Moves cursor to end of line like usua terminal usage
                        clear_and_redraw(shell_prompt, line, pos);
                        fflush(stdout);

                    }else if (seq[1] == 'A'){
                        if (current_hist == NULL) {
                            current_hist = history_tail;
                        } else if (current_hist->prev) {
                            current_hist = current_hist->prev;
                        }
                        if (current_hist) {
                            strncpy(line, current_hist->line, length - 1);
                            line[length - 1] = '\0'; // Safety
